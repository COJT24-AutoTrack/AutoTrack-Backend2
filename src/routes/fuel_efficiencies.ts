...
fuelEfficiencyRoutes.get('/', async (c) => {
    try {
        const fuelEfficiencies = await c.env.DB.prepare(`SELECT * FROM FuelEfficiencies`).all()
...
fuelEfficiencyRoutes.get('/:fe_id', async (c) => {
    try {
...
fuelEfficiencyRoutes.delete('/:fe_id', async (c) => {
    try {
        const fe_id = parseInt(c.req.param('fe_id'), 10)
        if (isNaN(fe_id)) {
            return c.json({ error: 'Invalid fe_id' }, 400)
